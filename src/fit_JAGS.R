...
po.dat <- otterDat %>%
  ...
  filter(!PA,
         as.logical(presence),
         grid.cell %in% L93_grid$grid.cell,
         !collision) %>%
...
MOD <- "ISDM"
mod <- run.jags(
  model = "JAGS/intSDMgam_JAGSmod.R",
  monitor = c(
    "z",
    "beta_region",
    "beta_latent",
...
outpath <-
  paste0("outMod/",
         paste(Sys.Date(), MOD, paste(regions, collapse = "."), tmp.res,
               sep = "_"),
         "yrs.rds")
saveRDS(out, outpath)
...
z.df <- data.frame(
  grid.cell = rep(L93_grid$grid.cell, nperiod),
  meanz = c(z[, 1:nperiod]),
  period = rep(unique(po.dat$period), each = nrow(z))
) %>%
  left_join(L93_grid, .) %>%
  mutate(period = paste0(period * tmp.res, " - ", period * tmp.res + tmp.res -
                           1))
...
beta_reg.df <-
  data.frame(
    period = rep(unique(po.dat$period), each = length(unique(regions))),
...
rho.df <-
  data.frame(protocole = rep(c("transect", "point"), each = nrow(beta_rho)),
...
otterDat.toplot <- otterDat %>%
  ...
  mutate(
    dataType = ifelse(
      PA & as.logical(presence),
      "PNA presence",
      ifelse(PA, "PNA absence", "presence Opportuniste")
    ),
    period = paste0(period * tmp.res, " - ", period * tmp.res + tmp.res -
                      1)
  )
...
ggplot(latent.df.peryear) +
  ...
  geom_ribbon(aes(x = period, ymin = psi.inf, ymax = psi.sup),
              alpha = 0.5,
              col = "black") +
  theme_bw()
